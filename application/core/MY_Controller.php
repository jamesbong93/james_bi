...
class MY_Controller extends CI_Controller
{
	public function __construct()
	{
        ...
        $this->data['custom_angular_dir']    = $this->config->item('custom_angular_dir');
...
class Member_Controller extends MY_Controller
{
    public function __construct()
    {
        ...
        if ( ! $this->ion_auth->logged_in() OR ! $this->ion_auth->is_member())
        {
            redirect('auth/login', 'refresh');
        }
        else
        {
            ...
            /* Load library function  */
            $this->breadcrumbs->unshift(0, 'Dashboard', 'member/dashboard');
...
class Senior_Controller extends MY_Controller
{
    public function __construct()
    {
        parent::__construct();

        if ( ! $this->ion_auth->logged_in() OR ! $this->ion_auth->is_senior())
        {
            redirect('auth/login', 'refresh');
        }
        else
        {
            /* Load */
            $this->load->config('senior/dp_config');
            $this->load->library('senior/page_title');
            $this->load->library('senior/breadcrumbs');
            $this->load->model('admin/core_model');
            $this->load->helper('menu');
            

            /* Load library function  */
            $this->breadcrumbs->unshift(0, 'Dashboard', 'senior/dashboard');
...
class Public_Controller extends MY_Controller
{
	public function __construct()
	{
        ...
        {
            $this->data['member_link'] = TRUE;
        }
        else
        {
            $this->data['member_link'] = FALSE;
        }

        if($this->ion_auth->logged_in() && $this->ion_auth->is_senior())
        {
            $this->data['senior_link'] = TRUE;
        }
        else
        {
            $this->data['senior_link'] = FALSE;
        }
